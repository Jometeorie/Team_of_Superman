...
//    超管输入的单日罚金数，设置系统的当日罚金（更新所有超管的单日罚金数据）
    ...
    double SelectFine(@Param("READER_ID") String adminId);

    //    根据当前超管的编号，获取当前系统的设置的图书期限
    @Select("SELECT DISTINCT DATE_SET FROM admin WHERE ADMIN_ID=#{adminId}")
    int SelectDate(@Param("READER_ID") String adminId);
    ...
    //根据当前用户编号，查询其所有已还书籍编号
    @Select("SELECT DISTINCT BOOK_ID FROM `return` WHERE READER_ID=#{readerId}")
    ResultSet SelectListReturnBookId(@Param("READER_ID")String readerId);

    //查询当前已借图书的书籍编号
    @Select("SELECT DISTINCT BOOK_ID FROM checked_out WHERE READER_ID=#{readerId}")
    ResultSet SelectListCheckedOutBookId(@Param("READER_ID")String readerId);
}
