...
CREATE TABLE `reserve` (     `RESV_ID` varchar(33) NOT NULL,     `BOOK_ID` varchar(32) NOT NULL,     `READER_ID` varchar(11) NOT NULL,     `BEGIN_TIME` datetime NOT NULL,     `END_TIME` datetime NOT NULL,     `STATUS` varchar(10) NOT NULL,     PRIMARY KEY (`RESV_ID`),     CONSTRAINT `reserve_fk_1` FOREIGN KEY (`BOOK_ID`) REFERENCES `book`(`BOOK_ID`),     CONSTRAINT `reserve_fk_2` FOREIGN KEY (`READER_ID`) REFERENCES `reader`(`READER_ID`) ) ENGINE=InnoDB DEFAULT CHARSET=utf8;

/*Data for the table `reserve` */

insert into `reserve` values(120,1,13512345678,now(),"2021-12-12 23:59:59","WAITING");
